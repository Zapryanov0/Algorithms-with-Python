def find_all_solutions(index, target, words_by_index, words_count, used_words):
    if index >= len(target):
        print(*used_words, ' ')
        return
    ...
    for word in words_by_index[index]:
        ...
        words_count[word] -= 1
        find_all_solutions(index + len(word), target, words_by_index, words_count, used_words)
